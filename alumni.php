...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!enrollCaptchaPassed()) $errors[] = 'Security check failed, please solve the sum and try again.';
    ...
    if (!$errors) {
        $stmt = $db->prepare("INSERT INTO alumni (name, achievement, batch_info, photo, story, contact, is_active, status) VALUES (?,?,?,?,?,?,0,'pending')");
...
// The dark achiever band is the same admin-managed "Results & Track Record"
// list shown on Home and Testimonials - one list, three pages, always in sync.
$trackRecords = getTrackRecords();
$stories = $db->query("SELECT * FROM alumni WHERE status = 'approved' AND is_active = 1 ORDER BY sort_order DESC, id DESC")->fetchAll();
$captchaQuestion = enrollCaptchaQuestion();
?>
<main class="page-alumni">
<?php renderPageHero('alumni'); ?>

<?php if ($trackRecords): ?>
<section class="dark trackrecord" id="achievers">
...
<nav class="jumpnav wrap reveal" aria-label="Section navigation">
  ...
  <?php if ($trackRecords): ?><a href="#achievers"><span>Our Achievers</span></a><?php endif; ?>
