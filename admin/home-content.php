...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($sectionForm === 'why_card') {
        ...
        $saveSetting = static function (string $key, string $value) use ($settingStmt): void {
            $settingStmt->execute([$key, $value]);
        };
        foreach ([
            'kicker', 'hero_photo_name', 'hero_photo_role',
            'hero_title', 'hero_subtitle', 'hero_micro',
            'hero_cta1_label', 'hero_cta1_link', 'hero_cta2_label', 'hero_cta2_link',
            'fc_popup_card_width', 'fc_popup_btn1_label', 'fc_popup_btn1_link',
            'fc_popup_btn2_label', 'fc_popup_btn2_link',
        ] as $key) {
        ...
        if (in_array($foundersTeacherId, $teacherIds, true)) {
            $saveSetting('founders_vision_teacher_id', (string)$foundersTeacherId);
        }
        ...
        try {
            ...
            if ($heroImage) {
                $saveSetting('hero_image', $heroImage);
            }
            ...
            if (isset($_POST['track_record_bg_remove'])) {
                $imageStmt->execute(['home', 'track_record_bg', null]);
            }
...
<nav class="admin-tabbar" role="tablist" aria-label="Homepage sections">
  ...
  <button type="button" class="admin-tab" data-tab-group="main" data-tab-target="why-cards" role="tab" aria-selected="false"><?= icon('award', 'tab-icon') ?> Why EnglishKeys <span class="tab-count"><?= count($whyCards) ?></span></button>
</nav>

<form method="post" enctype="multipart/form-data" class="admin-form" id="homeContentForm">
  <?= csrfField() ?>
  <input type="hidden" name="section_form" value="content">

  <div class="admin-tabpanel" id="hero" data-tab-group="main" data-tab-id="hero">
    <h2>Hero Section</h2>
    <label>Hero Eyebrow Label (small line above the title, e.g. "FBISE - Class 9-12 - Online")
      <input type="text" name="kicker" value="<?= e($settings['kicker'] ?? '') ?>">
    </label>
    <label>Hero Title
    ...
    <label>Photo Caption - Name (shown on the card under the hero photo)
      <input type="text" name="hero_photo_name" value="<?= e($settings['hero_photo_name'] ?? 'Mr. Naeem Haider') ?>">
    </label>
    <label>Photo Caption - Role (e.g. "Co-Founder &amp; Lead Instructor")
      <input type="text" name="hero_photo_role" value="<?= e($settings['hero_photo_role'] ?? 'Co-Founder & Lead Instructor') ?>">
    </label>
  </div>
  ...
  <div class="admin-tabpanel" id="track-record" data-tab-group="main" data-tab-id="track-record" hidden>
    ...
    <label><?= e($imageBlocks['track_record_bg']['label']) ?>
      <?php if (!empty($imageValues['track_record_bg'])): ?>
        ...
        <label class="checkbox-label"><input type="checkbox" name="track_record_bg_remove" value="1"> Remove this image</label>
      <?php endif; ?>
  ...
  <div class="admin-tabpanel" id="founders-vision" data-tab-group="main" data-tab-id="founders-vision" hidden>
    ...
    <div class="admin-note">
      <?= icon('document', 'note-icon') ?>
      <p>A quote-only section: section label, the quote, and the founder's name and title (this section is hidden automatically when the quote is empty). Name and title are pulled from <a href="teachers.php">Our Team &rarr;</a>, just pick which team member is credited below.</p>
    </div>
    <label>Section Label
      <input type="text" name="founders_heading" value="<?= e($blockValues['founders_heading']) ?>">
    </label>
    <label>Quote
      <textarea name="quote" rows="3"><?= e($blockValues['quote']) ?></textarea>
    </label>
  ...
  <div class="admin-tabpanel" id="popup" data-tab-group="main" data-tab-id="popup" hidden>
    ...
    <label>Button 1 Label
      <input type="text" name="fc_popup_btn1_label" value="<?= e($settings['fc_popup_btn1_label'] ?? 'Enroll Now') ?>">
    </label>
    <label>Button 1 Link
      <input type="text" name="fc_popup_btn1_link" value="<?= e($settings['fc_popup_btn1_link'] ?? 'contact.php') ?>">
    </label>
...
<div class="admin-tabpanel admin-section-card" id="why-cards" data-tab-group="main" data-tab-id="why-cards" hidden>
  ...
  <div class="admin-note">
    <?= icon('card', 'note-icon') ?>
    <p>The "Why EnglishKeys" section on the Home page: its heading, and the 3 (or more) cards below it. Add, edit, delete, reorder cards with Sort Order, and hide with Visible.</p>
  </div>

  <label>Heading
    <input type="text" name="why_heading" value="<?= e($blockValues['why_heading']) ?>" form="homeContentForm">
  </label>
