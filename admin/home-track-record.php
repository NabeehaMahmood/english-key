...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        try {
            $image = handleImageUpload('image', 'gallery');
        ...
        if ($id > 0) {
            ...
            $db->prepare('INSERT INTO track_records (year, position_badge, student_name, achievement_title, description, image, sort_order, is_active) VALUES (?,?,?,?,?,?,?,?)')
               ->execute([$year, $positionBadge, $studentName, $achievementTitle, $description, $image, $sortOrder, $isActive]);
...
<h1>Homepage Track Record</h1>
<p class="admin-page-intro">The "Proven Track Record" achiever cards, shown on the Home page ("Proven Track Record"), the Testimonials page ("Alumnus Corner") and the Alumni page (top band). One shared list, every page stays in sync automatically. Add, edit, delete, reorder with Sort Order, and hide with Visible.</p>

<nav class="admin-tabbar" role="tablist" aria-label="Homepage Track Record sections">
