...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        try {
            $photo = resolvePhotoChoice('photo_choice', 'photo', 'teachers');
        ...
        if ($id > 0) {
            if ($photo !== null) {
                ...
                $db->prepare('UPDATE teachers SET name=?, photo=?, role_title=?, qualification=?, subject=?, detail_bio=?, credentials=?, sort_order=?, is_active=? WHERE id=?')
                   ->execute([$name, $newPhoto, $roleTitle, $qualification, $subject, $detailBio, $credentials, $sortOrder, $isActive, $id]);
                ...
                $db->prepare('UPDATE teachers SET name=?, role_title=?, qualification=?, subject=?, detail_bio=?, credentials=?, sort_order=?, is_active=? WHERE id=?')
                   ->execute([$name, $roleTitle, $qualification, $subject, $detailBio, $credentials, $sortOrder, $isActive, $id]);
            }
            redirectWithMessage('teachers.php#' . $returnTab, 'Team member updated.');
        } else {
            $db->prepare('INSERT INTO teachers (name, photo, role_title, qualification, subject, detail_bio, credentials, sort_order, is_active) VALUES (?,?,?,?,?,?,?,?,?)')
               ->execute([$name, $photo ?: null, $roleTitle, $qualification, $subject, $detailBio, $credentials, $sortOrder, $isActive]);
...
function renderTeacherForm(?array $row, string $heading, string $returnTab, bool $allowCancel = false): void
{
    ...
    <form method="post" enctype="multipart/form-data" class="admin-form">
      ...
      <label>Qualification (shown as a hover tooltip on their card in About page "Meet Our Team", e.g. "M.Phil. English Linguistics")
        <input type="text" name="qualification" value="<?= e($row['qualification'] ?? '') ?>">
      </label>
      <label>Subject / Short Role
        <input type="text" name="subject" value="<?= e($row['subject'] ?? '') ?>">
      </label>
      <label>Biography (Founder &amp; Co-Founder only - the About page profile section, not shown for regular Faculty; separate paragraphs on blank lines)
        <textarea name="detail_bio" rows="6"><?= e($row['detail_bio'] ?? '') ?></textarea>
      </label>
      <label>Portfolio &amp; Qualifications (Founder &amp; Co-Founder only - one per line, shown as a list on the About page profile section, not shown for regular Faculty)
      ...
      <?= renderPhotoChoiceField('photo_choice', 'photo', $row['photo'] ?? null, 'Photo (upload a real photo, or pick an avatar if none is available)') ?>
