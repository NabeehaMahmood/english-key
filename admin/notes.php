...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_class') {
        ...
        if ($id > 0) {
            ...
            $classLevel = (int)($_POST['class_level'] ?? 0);
            if ($classLevel < 1 || $classLevel > 255) {
                redirectWithMessage($backTo, 'Class level must be a number between 1 and 255.', 'error');
            }
            $stmt = $db->prepare('SELECT id FROM note_classes WHERE class_level = ?');
            $stmt->execute([$classLevel]);
            if ($stmt->fetch()) {
...
<form method="post" class="admin-form">
  ...
  <?php else: ?>
    <label>Class Level (a number -- 9-12 for academic classes, or any unused number up to 255 for anything else)
      <input type="number" name="class_level" min="1" max="255" required>
    </label>
  <?php endif; ?>
