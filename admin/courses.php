...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_featured_intro') {
        $fiTitle = trim($_POST['featured_intro_title'] ?? '');
        $fiDesc = trim($_POST['featured_intro_description'] ?? '');
        $db->prepare(
            'INSERT INTO content_blocks (page_slug, block_key, content) VALUES (?, ?, ?)
             ON DUPLICATE KEY UPDATE content = VALUES(content)'
        )->execute(['courses', 'featured_intro', $fiTitle . '|' . $fiDesc]);
        redirectWithMessage('courses.php?category=featured', 'Featured section heading updated.');
    }
...
[$featuredIntroTitle, $featuredIntroDesc] = array_pad(explode('|', getContentBlock('courses', 'featured_intro')['content'] ?? '', 2), 2, '');
...
<div data-tab-panel="featured"<?= $activeCategory === 'featured' ? '' : ' hidden' ?>>
  <form method="post" class="admin-form">
    <?= csrfField() ?>
    <input type="hidden" name="action" value="save_featured_intro">

    <h2>Featured Section Heading</h2>
    <p class="admin-page-intro">The heading shown above the Featured Courses section on the public Courses page.</p>
    <label>Title
      <input type="text" name="featured_intro_title" value="<?= e($featuredIntroTitle) ?>">
    </label>
    <label>Description
      <input type="text" name="featured_intro_description" value="<?= e($featuredIntroDesc) ?>">
    </label>
